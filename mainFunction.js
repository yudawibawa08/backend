/**
 * Pranaya - Complete Dynamic E-commerce Core Engine Script
 */
    ...
    pdpContainer.innerHTML = `
        ...
        <style>
            ...
            @media (max-width: 768px) {
                ...
                .pdp-gallery img { border-radius: 8px !important; }
                .pdp-details-info h2 { font-size: 1.8rem !important; margin-top: 0.5rem !important; }
                .pdp-sticky-container {
...
// 8. Render Tampilan Formulir Lembar Checkout & Ekspedisi Dinamis Seluruh Indonesia
function renderCheckoutFormView() {
    ...
    appCartState.forEach(item => {
        ...
        totalWeightGram += (350 * item.qty); // Estimasi berat dasar: 350 gram per item
    ...
    const initialShippingCost = 0.00; 
    ...
    view.innerHTML = `
        ...
        <div class="checkout-container">
            <div class="checkout-card">
                ...
                <form id="purchaseSubmitForm" onsubmit="executeOrderFinalization(event)">
                    ...
                    <div class="form-field-control">
                        ...
                        <select id="checkoutCountrySelect" name="Destination Country" onchange="updateShippingCost(${itemSubtotal}, ${totalWeightGram})" required>
                            <option value="id">Indonesia (Domestic Shipping)</option>
                            <option value="sg">Singapore (International)</option>
                            <option value="au">Australia (International)</option>
                            <option value="us">United States (International)</option>
                        </select>
                    </div>

                    <!-- Dropdown Dinamis Seluruh Kota Se-Indonesia -->
                    <div class="form-field-control" id="indonesiaCityWrapper">
                        <label>Destination City (RajaOngkir Target)</label>
                        <select id="checkoutCitySelect" name="Destination City" onchange="updateShippingCost(${itemSubtotal}, ${totalWeightGram})">
            ...
            <div class="order-summary-box">
                ...
                <div class="summary-row-line">
                    <span>Shipping Fee<br><small style="color:#777;" id="shippingZoneMethod">Loading shipping details...</small></span>
                    <span id="checkoutShippingCostText">$ 0.00</span>
                </div>
                
                <!-- Box Notifikasi Informasi Email Internasional -->
                <div id="internationalShippingNotice" style="display:none; margin-top: 1rem; padding: 0.85rem; background: #fffdf0; border: 1px solid #f1e4b3; border-radius: 4px; font-size: 0.85rem; color: #7c6223; line-height: 1.4;">
                    ℹ️ <strong>International Delivery Note:</strong> The exact shipping invoice will be calculated manually and sent to your email after secure order checkout confirmation.
                </div>
    ...
    // Tarik data kota Indonesia saat pertama kali tampilan checkout dirender
    muatDaftarKotaRajaOngkir(itemSubtotal, totalWeightGram);
}

// 9. Fungsi Pengisian Opsi Dropdown Kota dari Backend
async function muatDaftarKotaRajaOngkir(itemSubtotal, totalWeightGram) {
    const citySelect = document.getElementById("checkoutCitySelect");
    if (!citySelect) return;

    try {
        const response = await fetch("http://localhost:3000/api/daftar-kota");
        const data = await response.json();

        if (data.success) {
            let optionsBuffer = "";
            data.results.forEach(city => {
                optionsBuffer += `<option value="${city.city_id}">${city.type} ${city.city_name} (${city.province})</option>`;
            });
            citySelect.innerHTML = optionsBuffer;
            
            // Kalkulasi ongkos kirim awal untuk kota pertama di daftar
            updateShippingCost(itemSubtotal, totalWeightGram);
        } else {
            citySelect.innerHTML = `<option value="">Failed to load cities</option>`;
        }
    } catch (error) {
        citySelect.innerHTML = `<option value="">Server connection error</option>`;
    }
}

// Fungsi Kalkulator Ongkir Gabungan RajaOngkir Domestik & Internasional via Email
async function updateShippingCost(itemSubtotal, totalWeightGram) {
    const countryKey = document.getElementById("checkoutCountrySelect").value;
    const citySelectWrapper = document.getElementById("indonesiaCityWrapper");
    const internationalNotice = document.getElementById("internationalShippingNotice");
    
    const usdToIdrRate = 16000; // Simulasi nilai konversi kurs mata uang
    let newShippingCost = 0;
    let trackingMethodText = "";

    if (countryKey === "id") {
        if (citySelectWrapper) citySelectWrapper.style.display = "block";
        if (internationalNotice) internationalNotice.style.display = "none";
        
        const cityId = document.getElementById("checkoutCitySelect").value;
        trackingMethodText = "Via JNE (RajaOngkir Domestik)";

        if (cityId) {
            try {
                const response = await fetch("http://localhost:3000/api/cek-ongkir-domestik", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ destinationCityId: cityId, totalWeightGram: totalWeightGram })
                });
                const data = await response.json();
                
                if (data.success) {
                    newShippingCost = data.ongkirRupiah / usdToIdrRate;
                } else {
                    newShippingCost = 3.00; // Fallback jika gagal hitung tarif
                }
            } catch (error) {
                newShippingCost = 3.00; // Fallback jika koneksi backend gagal
            }
        }
    } else {
        if (citySelectWrapper) citySelectWrapper.style.display = "none";
        if (internationalNotice) internationalNotice.style.display = "block";
        
        const targetCountries = { sg: "Singapore", au: "Australia", us: "United States" };
        trackingMethodText = `To ${targetCountries[countryKey]}<br><span style="color:#c69c6d; font-weight:bold;">(Invoice sent via Email)</span>`;
        
        newShippingCost = 0.00; // Biaya diset 0.00 di invoice web karena ongkir dihitung menyusul
    }

    const newGrandTotal = itemSubtotal + newShippingCost;

    document.getElementById("shippingZoneMethod").innerHTML = trackingMethodText;
    ...
    if(document.getElementById("hiddenShippingCost")) {
        document.getElementById("hiddenShippingCost").value = countryKey === "id" ? `$ ${newShippingCost.toFixed(2)}` : "Pending (Calculated via Email)";
    }
    if(document.getElementById("hiddenGrandTotal")) {
        document.getElementById("hiddenGrandTotal").value = `$ ${newGrandTotal.toFixed(2)}`;
    }
}
...
function executeOrderFinalization(event) {
    ...
    const countryKey = document.getElementById("checkoutCountrySelect").value;
    const cleanedShippingText = document.getElementById("checkoutShippingCostText").innerText;
    const cleanedGrandTotalText = document.getElementById("checkoutGrandTotalText").innerText;
    
    let itemSubtotal = 0;
    appCartState.forEach(item => { itemSubtotal += (item.price * item.qty); });
    
    const finalShippingCost = parseFloat(cleanedShippingText.replace("$ ", ""));
    const finalGrandTotal = parseFloat(cleanedGrandTotalText.replace("$ ", ""));
    const shippingMethodLabel = countryKey === "id" ? "JNE (RajaOngkir Domestik)" : "International Delivery (Pending Email Invoice)";
    ...
    .then(response => {
        if (response.ok) {
            renderThankYouReceiptView(receiptCartItems, itemSubtotal, finalShippingCost, finalGrandTotal, shippingMethodLabel);
            appCartState = [];
            saveCartState();
            refreshCartUI();
            ...
            if (submitBtn) { submitBtn.innerText = "Complete Secure Purchase"; submitBtn.disabled = false; }
        }
    })
    .catch(error => {
...
// Merender nota pembelian lengkap secara dinamis di halaman Thank You
        ...
        <div style="text-align: center; max-width: 600px; margin: 4rem auto; background: white; padding: 3rem; border-radius: 8px; box-shadow: 0 4px 25px rgba(0,0,0,0.05);">
            ...
            <div style="border: 1px solid var(--border); border-radius: 6px; padding: 1.5rem; margin-bottom: 2rem; background: #fdfbf7;">
                ...
                <div style="display: flex; justify-content: space-between; padding: 0.4rem 0; font-size: 0.9rem; color: #555; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 0.75rem;">
                    ...
                    <span>${shipping === 0 ? 'Pending Invoice via Email' : '$ ' + shipping.toFixed(2)}</span>
                </div>
